...
function SignForm({ title, inputs = [], submitButton, error, textUnderSubmit, link }) {
  ...
  return (
    ...
    <article className={'signForm'}>
      ...
      <form className='signForm__form'>
        ...
        {/* // ? сообщение о ошибке */}
        <p className='signForm__error-message'>
          {error}
        </p>
        ...
        <p className='signForm__text-under-submit'>
          {textUnderSubmit} <NavLink className={'signForm__link link'} to={link.to}>{link.text}</NavLink>
        </p>
